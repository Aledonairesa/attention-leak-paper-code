...
import joblib
from inject_mixed_df import inject_mixed_df
...
def untangle_tasks(dataset, model, alpha, beta,
                   process_time_column=False, process_hosts=True,
                   drop_dataset_id=True,
                   inject_handshakes=0, inject_len_spread=1.0):
    """
    ...
    # Inject synthetic handshakes (as part of mitigation experiments)
    if inject_handshakes > 0:
        df = inject_mixed_df(df, injections=inject_handshakes,
                            len_spread=inject_len_spread, silent=False)
...
def main():
    ...
    # ---- 3. Iterate models and α/β pairs ----
    for model_name, model_path in tqdm(cfg['models'], desc='Models'):
        ...
        for alpha, beta in tqdm(cfg['alpha_beta_pairs'],
            ...
            merged_assignments, metrics = untangle_tasks(
                mixed_df.reset_index(drop=True), model,
                alpha=alpha, beta=beta,
                process_time_column=False, process_hosts=True, drop_dataset_id=True,
                inject_handshakes=cfg.get("inject_handshakes", 0),
                inject_len_spread=cfg.get("inject_len_spread", 1.0),
            )
